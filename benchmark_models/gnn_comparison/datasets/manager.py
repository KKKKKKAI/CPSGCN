...
from dataset_utils import NumpyEncoder
...
class GraphDatasetManager:
    ...
    def get_model_selection_fold(self, outer_idx, inner_idx=None, batch_size=1, shuffle=True):
        ...
        print("trian", len(idxs["train"]))
        print(len(train_loader.dataset.get_data()))
        print(len(train_loader.dataset.indices))
        print(max(train_loader.dataset.indices))
...
class PlanetoidDatasetManager(GraphDatasetManager):
    ...
    def _process(self):
        dataset = Planetoid(root=f'data/Planetoid/{self.name}', name=self.name, transform=T.NormalizeFeatures())
        self.data = dataset[0]
    ...
    def _get_loader(self, dataset, batch_size=1, shuffle=True):
        # dataset = GraphDataset(data)
        sampler = RandomSampler(dataset) if shuffle is True else None
        print("using geometric loader")
        return torch_geometric.data.DataLoader(dataset,
                          batch_size=batch_size,
                          sampler=sampler,
                          shuffle=False,  
                          pin_memory=True)
    ...
    def get_test_fold(self, outer_idx, batch_size=1, shuffle=True):
        ...
        idxs = self.splits[outer_idx]["test"]
        test_data = GraphDatasetSubset([self.dataset[0]], idxs)
    ...
    def get_model_selection_fold(self, outer_idx, inner_idx=None, batch_size=1, shuffle=True):
        ...
        idxs = self.splits[outer_idx]["model_selection"][inner_idx]
        train_data = GraphDatasetSubset([self.dataset[0]], idxs["train"])
        val_data = GraphDatasetSubset([self.dataset[0]], idxs["validation"])

        train_loader = self._get_loader(train_data, batch_size, shuffle)

        print("iteration check")
        print("trian", len(idxs["train"]))
        print(train_loader.dataset.get_data())
        print(len(train_loader.dataset.indices))
        print(max(train_loader.dataset.indices))
...
class Cora(PlanetoidDatasetManager):
    name = "Cora"
    _dim_features = 1433
    _dim_target = 7
    max_num_nodes = 2708

class CiteSeer(PlanetoidDatasetManager):
    name = "Citeseer"
    _dim_features = 3703
    _dim_target = 6
    max_num_nodes = 3327
