...
class KFoldAssessment:
        ...
        for i in range(1, self.outer_folds+1):
            try:
                ...
                with open(config_filename, 'r') as fp:
                    outer_fold_accs = json.load(fp)

                    outer_TR_accs.append(outer_fold_accs['OUTER_TR'])
                    outer_TS_accs.append(outer_fold_accs['OUTER_TS'])

            except Exception as e:
                print(e)

        outer_TR_accs = np.array(outer_TR_accs)
        outer_TS_accs = np.array(outer_TS_accs)

        assessment_results['avg_TR_acc'] = outer_TR_accs.mean()
        assessment_results['std_TR_acc'] = outer_TR_accs.std()
        assessment_results['avg_TS_acc'] = outer_TS_accs.mean()
        assessment_results['std_TS_acc'] = outer_TS_accs.std()
    ...
    def _risk_assessment_helper(self, outer_k, experiment_class, exp_path, debug=False, other=None):
        ...
        for i in range(3):
            training_acc, test_acc = experiment.run_test(dataset_getter, logger, other)
            print(f'Final training run {i + 1}: {training_acc}, {test_acc}')

            training_accs.append(training_acc)
            test_accs.append(test_acc)

        training_acc = sum(training_accs) / 3
        test_acc = sum(test_accs) / 3

        logger.log('End of Outer fold. TR acc: ' + str(training_acc) + ' TS acc: ' + str(test_acc))

        with open(os.path.join(exp_path, self._OUTER_RESULTS_FILENAME), 'w') as fp:
            json.dump({'best_config': best_config, 'OUTER_TR': training_acc, 'OUTER_TS': test_acc}, fp)
