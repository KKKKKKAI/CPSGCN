...
class KFoldSelection:
    ...
    def process_results(self, KFOLD_FOLDER, no_configurations):
        ...
        for i in range(1, no_configurations+1):
            try:
                ...
                avg_vl = config_dict['avg_VL_acc']
                std_vl = config_dict['std_VL_acc']
    ...
    def _model_selection_helper(self, dataset_getter, experiment_class, config, exp_config_name,
        ...
        for k in range(self.folds):
            ...
            training_acc, validation_acc = experiment.run_valid(dataset_getter, logger, other)

            logger.log(str(k+1) + ' split, TR Accuracy: ' + str(training_acc) +
                       ' VL Accuracy: ' + str(validation_acc))

            k_fold_dict['folds'][k]['TR_acc'] = training_acc
            k_fold_dict['folds'][k]['VL_acc'] = validation_acc

        tr_accs = np.array([k_fold_dict['folds'][k]['TR_acc'] for k in range(self.folds)])
        vl_accs = np.array([k_fold_dict['folds'][k]['VL_acc'] for k in range(self.folds)])

        k_fold_dict['avg_TR_acc'] = tr_accs.mean()
        k_fold_dict['std_TR_acc'] = tr_accs.std()
        k_fold_dict['avg_VL_acc'] = vl_accs.mean()
        k_fold_dict['std_VL_acc'] = vl_accs.std()

        logger.log('TR avg is ' + str(k_fold_dict['avg_TR_acc']) + ' std is ' + str(k_fold_dict['std_TR_acc']) +
                   ' VL avg is ' + str(k_fold_dict['avg_VL_acc']) + ' std is ' + str(k_fold_dict['std_VL_acc']))
