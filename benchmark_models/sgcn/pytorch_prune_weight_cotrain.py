...
# print("-----------Start Weight-Graph Pruning---------------")
support1 = model.adj1
support2 = model.adj2
...
for j in range(args.times):
    for epoch in range(args.epochs):
        ...
        train_loss = (loss(model) + \
            rho * (F.mse_loss(support1 + U1, Z1 + torch.eye(support1.shape[0]).to(device)) +
            F.mse_loss(support2 + U2, Z2 + torch.eye(support2.shape[0]).to(device)))).item()
        
        # Update to correct gradient
        update_gradients_adj(adj_map, partial_adj_mask)
        # Use the optimizer to update adjacency matrix
        adj_optimizer.step()
        weight_optimizer.step() # Update weight here?

        train_acc, val_acc, tmp_test_acc = test(model, data)
        if val_acc > best_prune_acc:
            best_prune_acc = val_acc
            test_acc = tmp_test_acc
        log = 'Pruning Epoch: {:03d}, Train: {:.4f}, Val: {:.4f}, Test: {:.4f}, {:.4f}'
        print(log.format(epoch, train_acc, val_acc, tmp_test_acc, train_loss))
...
torch.save({"state_dict":model.state_dict(),"adj":cur_adj1}, "./prune_weight_cotrain/model.pth.tar")
